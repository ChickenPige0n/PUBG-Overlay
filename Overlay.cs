...
internal class PubgOverlayRenderer : Overlay
{
    ...
    protected override void Render()
    {
        ...
        if (_downloadProgress.HasValue)
        {
            drawList.AddRectFilled(new Vector2(0, 0), new Vector2((int)(_downloadProgress * Size.Width), 20),
                GetColor(1));
        }
        ...
        if (_showSettings)
        {
            ImGui.PushStyleColor(ImGuiCol.WindowBg,
                _showSettings ? new Vector4(0.3f, 0.2f, 0.3f, 0.4f) : new Vector4(0.3f, 0.2f, 0.3f, 0.0f));
            ...
            if (_updateUrl != null)
            {
                ...
                if (ImGui.Button("去下载"))
                {
                    ...
                    UpdateHelper.Update(_updateUrl, progress);
                }
            }
        ...
        ImGui.Begin("DistanceDisplay",
            ImGuiWindowFlags.NoDecoration | ImGuiWindowFlags.NoMove | ImGuiWindowFlags.NoInputs |
        ...
        if (_playerPos.HasValue)
        {
            ...
            drawList.AddRect(_playerPos.Value - size * new Vector2(0.5f, 1),
                _playerPos.Value + size * new Vector2(0.5f, 0f), GetColor(_playerTeam));
        }

        if (_targetPos.HasValue)
        {
            var size = _openCvManager.TargetTemplateSize(_targetTeam - 1);
            drawList.AddRect(_targetPos.Value - size * new Vector2(0.5f, 1),
                _targetPos.Value + size * new Vector2(0.5f, 0f), GetColor(_targetTeam));
        }

        if (_distance > 100)
        {
            drawList.AddLine(new Vector2(Size.Width / 2.0f, 0), new Vector2(Size.Width / 2.0f, Size.Height),
                GetColor(_playerTeam), 1);
            drawList.AddLine(new Vector2(Size.Width / 2.0f, Size.Height / 2.0f),
                new Vector2(Size.Width / 2.0f + 15, Size.Height / 2.0f), GetColor(_playerTeam), 1);
            drawList.AddText(new Vector2(Size.Width / 2.0f + 24, Size.Height / 2.0f - 10), GetColor(_playerTeam),
                $"{_distance:F2}");
            ...
            for (var i = -screenSegments; i <= screenSegments; i++)
            {
                ...
                var point = new Vector2(Size.Width / 2.0f,
                    Size.Height / 2.0f + Size.Height / 2.0f / screenSegments * i);
                drawList.AddLine(
                    point,
